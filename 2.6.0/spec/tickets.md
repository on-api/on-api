...
**API Prefix:** `/tickets` 
eg: `domain.local/api/2.6/tickets/423323449`
...
Request ticket:
```http
GET /api/2.6/tickets/{coTicketReference} 
```
...
```http
GET /api/2.6/tickets?status=IN_PROGRESS_CO 
```
...
```http
PATCH /api/2.6/tickets/{coTicketReference}
Content-Type: application/json
